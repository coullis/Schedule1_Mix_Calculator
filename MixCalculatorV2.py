...
def mix(prod, n):
    ...
    for combo in tqdm(combinations, total=16**n, ncols = 100, colour="cyan", desc="Processing combinations"):
        ...
        #result[c] = (c, len(substances), prod, substances, currentEffects, cost, round(finalPrice), round(finalPrice)-cost)
        if round(finalPrice)-cost > maxProfit:
            result = ((c, len(substances), prod, substances, currentEffects, cost, round(finalPrice), round(finalPrice)-cost))
            maxProfit = round(finalPrice)-cost
        else:
            c+=1
    return result
...
substances: 10 - iterations: 1099511627776 - time: 3000:00:00 - 3500:00:00  (estimation)
